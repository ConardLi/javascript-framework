<script>
  ...
  import { zhTranslations } from "./lib/i18n/zh.js";
  import { codeExplanations } from "./lib/i18n/codeExplanation.js";
  import { languageStore } from "./lib/stores/languageStore.js";
  import { enExplanations } from "./lib/i18n/enExplanation.js";
  ...
  const SITE_TITLE = "Learn JS Framework";
  ...
  function getSectionTitle(sectionId) {
    return $languageStore
      ? zhTranslations.sections[sectionId]?.title || sectionId
      : sectionId;
  }

  function getSnippetTitle(sectionId, snippetId) {
    return $languageStore
      ? zhTranslations.sections[sectionId]?.snippets[snippetId]?.title ||
          snippetId
      : snippetId;
  }

  function getSnippetDescription(sectionId, snippetId) {
    if (!$languageStore) return ""; // 英文模式下不显示描述
    return (
      zhTranslations.sections[sectionId]?.snippets[snippetId]?.description || ""
    );
  }

  // 根据语言获取说明
  function getExplanation(frameworkId, snippetId) {
    return $languageStore
      ? codeExplanations[frameworkId]?.[snippetId]?.explanation
      : enExplanations[frameworkId]?.[snippetId]?.explanation;
  }

  function getFileExplanation(frameworkId, snippetId, fileName) {
    return $languageStore
      ? codeExplanations[frameworkId]?.[snippetId]?.code?.[fileName]
          ?.explanation
      : enExplanations[frameworkId]?.[snippetId]?.code?.[fileName]?.explanation;
  }
</script>
          ...
          <div class="space-y-20">
            {#each sections as section}
              <div class="px-6 md:px-14 lg:px-20 max-w-full">
                ...
                >
                  {getSectionTitle(section.sectionId)}
                  <a
                ...
                <div class="space-y-8 mt-2">
                  {#each snippets.filter((s) => s.sectionId === section.sectionId) as snippet}
                    ...
                    <div id={snippetPathId} data-snippet-id={snippetPathId}>
                      ...
                      >
                        {getSnippetTitle(section.sectionId, snippet.snippetId)}
                        <a
                          href={"#" + snippetPathId}
                          aria-hidden="true"
                          tabindex="-1"
                      ...
                      {#if $languageStore && getSnippetDescription(section.sectionId, snippet.snippetId)}
                                ...
                                {#if frameworkSnippet}
                                  {#if frameworkSnippet.files.length > 0}
                                    <CodeEditor
                                      ...
                                      fileExplanations={Object.fromEntries(
                                        frameworkSnippet.files.map((file) => [
                                          file.fileName,
                                          getFileExplanation(
                                            framework.id,
                                            snippet.snippetId,
                                  ...
                                  {@const frameworkExplanation = getExplanation(
                                    framework.id,
                                    snippet.snippetId
                                  )}
                                  {#if frameworkExplanation}
