<script>
  ...
  function toggleLanguage() {
    ...
    localStorage.setItem("showChineseExplanation", $languageStore);
    notifications.show({
      title: $languageStore
        ? "已切换到中文说明"
        : "Switched to English explanation",
    });
  }
</script>
    ...
    <div class="flex justify-between items-center py-3">
      ...
      <div class="flex items-center space-x-4">
        ...
        <button
          ...
          aria-label={$languageStore ? "切换到英文" : "Switch to Chinese"}
          onclick={toggleLanguage}
        >
          <span>{$languageStore ? "中" : "En"}</span>
        </button>
