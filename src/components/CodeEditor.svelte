<script>
  ...
  const { files = [], fileExplanations = {}, snippetEditHref = "" } = $props();
...
{#if fileExplanations[filenameSelected]}
  <div class="bg-gray-800 p-3 text-sm text-gray-300 rounded-t">
    {fileExplanations[filenameSelected]}
  </div>
{/if}
